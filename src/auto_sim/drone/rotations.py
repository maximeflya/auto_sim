...
| Description: Implements utilitary rotations between ENU and NED inertial frame
  conventions and FLU and FRD body frame conventions.
"""

import numpy as np
from scipy.spatial.transform import Rotation

# Quaternion for rotation between ENU and NED INERTIAL frames NED to ENU: +PI/2 rotation
# about Z (Down) followed by a +PI rotation around X (old North/new East) ENU to NED:
# +PI/2 rotation about Z (Up) followed by a +PI rotation about X (old East/new North) This
# rotation is symmetric, so q_ENU_to_NED == q_NED_to_ENU.  Note: this quaternion follows
# the convention [qx, qy, qz, qw]
